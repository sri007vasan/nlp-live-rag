...
* **LlamaIndex** provides search capability to Google Gemini LLM and combines functionalities such as chat memory and model calls for the app.
...
## Prerequisites
- A Google API Key (set `GOOGLE_API_KEY`) for Gemini access
...
Create a `.env` file under the root folder; set `GOOGLE_API_KEY` to allow the demo to call Google Gemini for answers.
